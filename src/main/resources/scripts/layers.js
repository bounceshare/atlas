<script>
    ...
    function refreshLayers(coords, radius) {
        ...
        httpPost(searchUrl, data, function(response) {
            invalidateMap(response.data.markers, response.data.fences, response.data.circles, response.data.paths, response.data.events, response.data.form, response.data.isSidebar, false, response.data.autoRefresh);
            showLoader(false);
